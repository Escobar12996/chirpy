...
jQuery(function($){
	...
	$(window).scroll(function() {
		...
		} else if(document.URL.split(window.location.host)[1].includes("/administration/images") && $(window).scrollTop() + $(window).height() == $(document).height() && finish === false){
                    let value = $('.last').last().attr('id');

                    $.ajax({
                            type: "POST",
                    url: "/refill/adminfimaall",
                    data : { find: 0 , last : value },
                    beforeSend: function(xhr) {
                        xhr.setRequestHeader(header, token);
                    },
